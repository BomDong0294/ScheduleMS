...
import ButtonActionListener.RestingAddListener;
import ButtonActionListener.ShoppingAddListener;
import ButtonActionListener.StudyingAddListener;
...
public class AdderMenu extends JPanel { // JFrame 클래스 상속
	...
	public AdderMenu(WindowFrame frame, ScheduleMethod schfunction) {
		this.frame = frame;
		...
		button1.addActionListener(new MeetingAddListener(frame));
		button2.addActionListener(new ShoppingAddListener(frame));
		button3.addActionListener(new StudyingAddListener(frame));
		button4.addActionListener(new RestingAddListener(frame));
